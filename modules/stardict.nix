...
let dictionaries = {
  ...
  makeStardictNonInteractive = name: dicts:
    pkgs.writers.writeDashBin name ''
      set -efu
      exec ${pkgs.sdcv}/bin/sdcv --non-interactive --exact-search --only-data-dir --data-dir ${makeStardictDataDir dicts} "$@" | ${toString sdcvPruneTags}
    '';

  sdcvPruneTags = pkgs.writers.writeDash "sdvcPruneTags" ''
  ...
  sdcvPager = pkgs.writers.writeDash "sdcvPager" ''
    export PATH=${lib.makeBinPath [pkgs.less]}
