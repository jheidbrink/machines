...
in
{
  ...
  environment.systemPackages = with pkgs; [
    ...
    jq
    curl
    (python3.withPackages (ps: [ ps.pyyaml ps.ipython ps.numpy ps.requests ]))  # putting ipython and yaml in the same environment so that I can `import yaml' from an ipython session
