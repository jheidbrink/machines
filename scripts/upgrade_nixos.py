...
result = subprocess.run(['git', 'rev-list', '-n', '1', f'--before={before}', 'nixos-unstable'], cwd=nixpkgs_path, check=True, capture_output=True, text=True)

new_rev = result.stdout.strip()

new_commit_date_s = subprocess.run(['git', 'show', '-s', '--format=%ci', new_rev], cwd=nixpkgs_path, capture_output=True, text=True, check=True).stdout.strip()

new_date = datetime.strptime(new_commit_date_s, '%Y-%m-%d %H:%M:%S %z')
print(new_date)


new_sha = subprocess.run(['nix-prefetch-url', '--unpack', f'https://github.com/nixos/nixpkgs/archive/{new_rev}.tar.gz'], capture_output=True, text=True, check=True).stdout.strip()


new_contents = contents.replace(rev, new_rev).replace(sha256, f'"{new_sha}"').replace(date_s, new_date.strftime("%Y-%m-%d"))
