...
echo -n $luks_passphrase | cryptsetup --batch-mode luksFormat ${device}p2 -
echo -n $luks_passphrase | cryptsetup --batch-mode open ${device}p2 encpv -

echo "creating volume group an logical volumes on physical volume /dev/mapper/encpv"
pvcreate /dev/mapper/encpv
vgcreate encvg /dev/mapper/encpv
